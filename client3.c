...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		...
		strcpy(sendline, "DELETE from EMPLOYEE where ID=5;");
		fprintf(fp, "Request: DELETE from EMPLOYEE where ID=5;\n");
		printf("%s\n", sendline);
		fputs(sendline, fp);
		...
		fprintf(fp, "String received from the server: \n");
		...
		strcpy(sendline, "INSERT INTO COMPANY (ID,NAME,AGE,ADDRESS,SALARY) VALUES (5, 'John5', 50, 'Texas', 90000.00);");

		fprintf(fp, "Request: INSERT INTO COMPANY (ID,NAME,AGE,ADDRESS,SALARY) VALUES (5, 'John5', 50, 'Texas', 90000.00);");
		printf("%s\n", sendline);
		send(sockfd, (const void*)sendline, strlen(sendline), 0);
